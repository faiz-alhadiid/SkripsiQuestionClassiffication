...
def find_evaluate(C, tol, max_iter):
    ...
    for i in range(5):
        ...
        svm = MultiClassSVM(C=C, tol=tol, max_iter=max_iter)
        svm.train(x_train, coarse_train)
        print()
        hasil =svm.classify_all(x_test)
        ev = evaluate(coarse_test, hasil, label=[key for key in svm.classifier])
...
def find_best_c():
    ...
    all_param.sort(key = lambda x: - x['evaluation']['avg_f_measure'])
